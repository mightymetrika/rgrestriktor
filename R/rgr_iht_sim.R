...
rgr_iht_sim <- function(Its = 1000,
  ...
  # store results
  res <- vector(mode = "list", length = Its)
  ...
  for (i in seq_len(Its)) {
    ...
    # (2) ANOVA & pairwise t-tests on intercepted formula
    outcome   <- all.vars(.formula)[1]
    rhs_terms <- attr(stats::terms(.formula), "term.labels")
    if (length(rhs_terms) != 1L) stop("Expecting a single grouping factor.")
    group_var <- rhs_terms[1]
    ...
    # enrich pairwise with direction/adjacency for gatekeeper logic
    outcome   <- all.vars(.formula)[1]
    rhs_terms <- attr(terms(.formula), "term.labels")
    group_var <- rhs_terms[1]
    ...
    # run IHT (now safe) and combine outputs
    rgr_iht_res <- rgr_iht(mod_iht, rgconstraints = rgconstraints, constraints = constraints)

    rgr_iht_res$iht_df <- dplyr::bind_rows(
      # ANOVA row (not an IHT fit): carry fit_ok = NA
      data.frame(
        modname = "anova",
      ...
      # pairwise rows (not an IHT fit): carry fit_ok = NA
      as.data.frame(pttest) |>
        dplyr::mutate(fit_ok = NA),
      # IHT and RG-IHT rows (each has fit_ok TRUE/FALSE from rgr_iht())
      rgr_iht_res$iht_df
    ) |>
      dplyr::mutate(N = nrow(dat)) |>
      dplyr::select(N, dplyr::everything())
